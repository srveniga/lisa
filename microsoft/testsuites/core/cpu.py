...
class CPU(TestSuite):
    ...
    def cpu_count_check(self, node: Node, log: Logger) -> None:
        ...
        # If there is cluster topology, calculate vCPU count by core_per_cluster_count *
        # cluster_count * thread_per_core_count, else by core_per_socket_count *
        # socket_count * thread_per_core_count.
        if lscpu.check_cluster_exists():
            calculated_cpu_count = (
                lscpu.get_core_per_cluster_count()
                * lscpu.get_cluster_count()
